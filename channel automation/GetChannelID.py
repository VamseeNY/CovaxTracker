"""
Author: Vamsee NY 
Description: This script was designed to get the channel ID of telegram channels. 
"""
...
def getid(j):
    driver.find_element_by_xpath('/html/body//*[@id="telegram-search-input"]').click()          #Clicking on the channel name input field
    driver.find_element_by_xpath('/html/body//*[@id="telegram-search-input"]').send_keys(n+j)   #Entering the channel name. Sample example: Covaxin_Jaipur
    time.sleep(5)
    pg.click(400,480)#moveTo                                                                    #Accessing the channel
    time.sleep(1)
    finaldata[str(j)]='-100'+str(driver.current_url)[-10:]                                      #When accessing the channel, A part of the web url displays the channel ID(which start with -100). Hence I copy the web url, slice it and add '-100' to it. 
...
names=[]
for a in ind:
    districts=cowin.get_districts(a.get("state_id"))      #Gettings a list of districts for the state a
    ins=list(districts.values())[0]
    for b in ins:
        names.append(str(b.get("district_name")))          #Gettings a list of district names
